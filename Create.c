Create()
{
	...
	status=web_submit_data("reservations.pl", 
		...
		"Name=depart", "Value={city2}", ENDITEM, 
	...
	if(j==0){
		lr_output_message("Билетов с четной ценой нет!");
	}else{	
		lr_output_message("Билеты с чётной ценой = %d",j);
	...
	srand(time(NULL));
 	...
 	lr_output_message("Выбран этот билет = %s",trueticket[randomize]);
 	 ...
 	 tName = "005.Create_ticket_Cost_2";
	 lr_start_transaction(tName);
	...
	status=web_submit_data("reservations.pl_2", 
		...
		tName = "006.Create_ticket_PaymentDetail_2";
		lr_start_transaction(tName);
